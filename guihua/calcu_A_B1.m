...
A31=zeros(length(bz.V),1);
A32=-1./(cos(theta)).^2;
A33=zeros(length(bz.V),1);
B11=D_alpha./(m.*V.*cos(theta));
B12=zeros(length(bz.V),1);
B21=-L_alpha.*cos(bank)./(m.*V.^2.*cos(theta));
B22=L.*sin(bank)./(m.*V.^2.*cos(theta));
B31=zeros(length(bz.V),1);
B32=zeros(length(bz.V),1);
...
A=zeros(3,3,length(bz.V));
B=zeros(3,2,length(bz.V));
for ii=1:length(bz.V)
    A(1,1,:)=A11;
    A(1,2,:)=A12;
    A(1,3,:)=A13;
    A(2,1,:)=A21;
 ...
 K=zeros(2,3,length(bz.V));
 P=zeros(3,3,length(bz.V));
 pdot=zeros(3,3,length(bz.V));
for ii=1:length(bz.V)
...
K1=reshape(K(1,:,:),3,length(bz.V)).';
K2=reshape(K(2,:,:),3,length(bz.V)).';
