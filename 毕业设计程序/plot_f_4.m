...
legend({'\rho无偏差','\rho+10%','\rho-10%'},'FontSize',9,'Location','NorthWest');
print -dtiffn -r600 比较hv14

figure(2);
plot(57.3*yout(:,2),57.3*yout(:,3),'b-.','LineWidth',0.8);
legend({'\rho无偏差','\rho+10%','\rho-10%'},'FontSize',9);
print -dtiffn -r600 比较地轨14

figure(3);
plot3(57.3*yout(:,2),57.3*yout(:,3),(yout(:,1)-Re)/1000,'b-.','LineWidth',1.2);
legend({'\rho无偏差','\rho+10%','\rho-10%'});