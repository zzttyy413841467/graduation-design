...
legend({'\rho无偏差','\rho+15%','\rho-15%'},'FontSize',9,'Location','NorthWest');
print -dtiffn -r600 比较hv24

figure(2);
plot(57.3*yout(:,2),57.3*yout(:,3),'b-.','LineWidth',0.8);
legend({'\rho无偏差','\rho+15%','\rho-15%'},'FontSize',9);
print -dtiffn -r600 比较地轨24

figure(3);
plot3(57.3*yout(:,2),57.3*yout(:,3),(yout(:,1)-Re)/1000,'b-.','LineWidth',1.2);
legend({'\rho无偏差','\rho+15%','\rho-15%'});